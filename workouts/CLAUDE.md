...
- **Warmup/cooldown power continuity** — Warmup `PowerHigh` must match the power of the next block. Cooldown `PowerLow` must match the power of the previous block. Exception: HIIT warmups ending before jumps to 85%+ are intentionally lower (you don't warmup to threshold/VO2max power).
